...
export async function GET(request: Request) {
  try {
    ...
    const rows = (tasks ?? [])
      .map(task => ({ task, taskRecords: recordsByTask.get(task.id) ?? [] }))
      .filter(({ taskRecords }) => taskRecords.reduce((sum, record) => sum + Number(record.horas), 0) > 0)
    ...
    // Toda la información se genera en una única hoja.
    ...
    sheet.columns = [
      ...
      { key: 'total', width: 16 },
      { key: 'title', width: 38 },
      { key: 'recordDate', width: 20 },
      { key: 'recordUser', width: 24 },
      { key: 'recordHours', width: 16 },
      { key: 'work', width: 55 },
    ]
    ...
    title.border = { bottom: { style: 'thin', color: { argb: 'FF1F4E78' } } }
    sheet.getRow(1).height = 30

    sheet.getCell('A2').value = 'Periodo'
    sheet.getCell('B2').value = period
    sheet.getCell('D2').value = 'Tareas cerradas'
    sheet.getCell('E2').value = details.length
    sheet.getCell('G2').value = 'Total horas'
    sheet.getCell('H2').value = totalHours
    ...
    header.eachCell(cell => {
      ...
      cell.border = {
        top: { style: 'thin', color: { argb: 'FFC8D0DA' } },
        bottom: { style: 'thin', color: { argb: 'FFC8D0DA' } },
        left: { style: 'thin', color: { argb: 'FFC8D0DA' } },
        right: { style: 'thin', color: { argb: 'FFC8D0DA' } },
      }
    })
    ...
    for (const detail of details) {
      detail.taskRecords.forEach((record, index) => {
        const row = sheet.addRow([
          index === 0 ? date(detail.task.fecha_creacion) : '',
        ...
        row.eachCell(cell => {
          ...
          cell.border = {
            top: { style: 'thin', color: { argb: 'FFD7DDE5' } },
            bottom: { style: 'thin', color: { argb: 'FFD7DDE5' } },
            left: { style: 'thin', color: { argb: 'FFD7DDE5' } },
            right: { style: 'thin', color: { argb: 'FFD7DDE5' } },
          }
        })
    ...
    sheet.mergeCells(`A${summaryTitleRow.number}:J${summaryTitleRow.number}`)
    const summaryTitle = summaryTitleRow.getCell(1)
    summaryTitle.value = 'DESGLOSE DE HORAS POR CENTRO DE COSTE'
    summaryTitle.font = { size: 12, bold: true, color: { argb: 'FFFFFFFF' } }
    summaryTitle.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF34495E' } }
    summaryTitle.alignment = { vertical: 'middle' }

    sheet.addRow([])
    const summaryHeader = sheet.addRow(['Centro de coste', 'Horas'])
    ...
    summaryHeader.getCell(1).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF34495E' } }
    summaryHeader.getCell(2).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF34495E' } }
    summaryHeader.getCell(1).alignment = { vertical: 'middle' }
    summaryHeader.getCell(2).alignment = { horizontal: 'right', vertical: 'middle' }

    for (const [name, hours] of [...hoursByCenter.entries()].sort((a, b) => a[0].localeCompare(b[0], 'es'))) {
      const row = sheet.addRow([name, hours])
    ...
    const summaryTotal = sheet.addRow(['TOTAL', totalHours])
    summaryTotal.getCell(1).font = { bold: true }
    summaryTotal.getCell(2).font = { bold: true }
    summaryTotal.getCell(2).numFmt = '0.00'
    summaryTotal.getCell(2).alignment = { horizontal: 'right' }
    ...
    // Filtro únicamente sobre la tabla principal.
