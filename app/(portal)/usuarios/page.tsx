  ...
  return <div className="stack">
    ...
    <div className="card"><div className="table-wrap"><table className="table"><thead><tr><th>Usuario</th><th>Rol</th><th>Centro</th><th>Estado</th><th>Auth</th><th>Acciones</th></tr></thead><tbody>
      {users.map(user => <tr key={user.id}>
        <td><strong>{user.nombre}</strong><div className="small muted">{user.email}</div><div className="small muted">{user.puesto || 'Sin puesto'}{user.departamento ? ` · ${user.departamento}` : ''}</div></td><td><span className="badge slate">{user.rol}</span></td><td>{user.centros?.nombre || '—'}</td>
